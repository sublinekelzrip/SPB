...
#define SET_BASIC_PARAMETERS	{ 	f[0] = 0x11b ; Highest_time_of_f = 8;					\
...
#define txt_name "testh_all_hdm.txt" 

//87610 
int mul[260][260],exprAB[260][8][8][10],now[260][8][8],ny[260];
...
int count_XOR(DWORD dw)
{
	...
	for(int i = 0; i < expr_size; i++)
	{
		...
		for(int j = 0; j < expr_size; j++)
		{
			...
			for(int k = 0; k < ele_num; k++)
			{
				if(exprAB[1][i][j][k] == -1) break;
				if(arr[exprAB[1][i][j][k]] == 1) XOR_num_per_ele ^= 1;
			}
			XOR_num_per_row += XOR_num_per_ele;
		}
		XOR_num += XOR_num_per_row;
...
int main(){
	...
	for(int tmp=0;tmp<30;tmp++)
	{
		...
		for(int R=1;R<=1;R++)
		{
			...
			if(cnt>0&&pb_vec[R][0].XOR_num_all<=336&&pb_vec[R][0].XOR_delay_all<=6)
				fout<<endl<<endl<<"******************"<<f[0]<<"******************"<<R<<"******************"<<endl<<endl;
			for(int i = 0; i < cnt; i++)
			{
				if(pb_vec[R][i].XOR_num_all <=336 && pb_vec[R][i].XOR_delay_all <= 6)  // 152 + 440
