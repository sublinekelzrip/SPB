...
int DWSZ, BLK_NUM, Highest_time_of_f, v;

BYTE TBL[65536], TBH[65536];

#define SET_BASIC_PARAMETERS	{ 	f[0] = 379; v = 16; Highest_time_of_f = 8;					\
...
#define txt_name "17b_8654310_SPB_Hadamard.txt" 
...
void count_XOR_num(DWORD dw, int order, int is_vec_in, int row)
{
	...
	XOR_num -= 8;
	if(is_vec_in == 1) spb_vec[order].XOR_num_in += XOR_num;
	else if(is_vec_in == 0) spb_vec[order].XOR_num_out += XOR_num;	
}
...
void inverse_vec(DWORD input_array[Matrix_size])
{
	...
	DWORD MI_a[MAXDWSZ] = {0}; DWORD b[MAXDWSZ] = {0};
    ...
    /*  debug  */
	// if(spb_vec[cnt].XOR_num_all <= 592 && spb_vec[cnt].XOR_delay_all <= 8)
	// {
	// 	for(int i = 0; i < Matrix_size; i++)
	// 	{
	// 		for(int j = 0; j < Matrix_size; j++)
	...
	for(int i = 0; i < Matrix_size; i++)
	{
		...
		spb_vec[cnt].vec_in += str;	
		if(i != Matrix_size - 1) spb_vec[cnt].vec_in += " ";
	}
	spb_vec[cnt].vec_in += ") ";
	spb_vec[cnt].XOR_num_in = 4 * (spb_vec[cnt].XOR_num_in + 3 * 8);
	spb_vec[cnt].XOR_delay_in = count_XOR_delay();
    ...
    for(int j = 0; j < Matrix_size; j++)
    {
    	...
    	spb_vec[cnt].vec_out += str;
    	if(j != Matrix_size - 1) spb_vec[cnt].vec_out += " ";
    }
    spb_vec[cnt].vec_out += ") ";
    spb_vec[cnt].XOR_num_out = 4 * (spb_vec[cnt].XOR_num_out + 3 * 8);
    spb_vec[cnt].XOR_delay_out = count_XOR_delay();

    spb_vec[cnt].XOR_num_all = spb_vec[cnt].XOR_num_in + spb_vec[cnt].XOR_num_out;
    spb_vec[cnt].XOR_delay_all = spb_vec[cnt].XOR_delay_in + spb_vec[cnt].XOR_delay_out;
